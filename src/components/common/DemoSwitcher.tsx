...
export const DemoSwitcher: React.FC<DemoSwitcherProps> = ({ onNavigateToAdmin, onNavigateToVendor, onNavigateToCustomer }) => {
  const { user, isAdmin, isVendor, switchDemoRole, logout } = useAuth();
  const [isOpen, setIsOpen] = useState(false);

  const getRoleLabel = () => {
    if (!user) return 'Guest';
    if (isAdmin) return 'Admin (Kwame)';
    if (isVendor) return 'Seller (Kofi)';
    return 'Customer (Abena)';
  };
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="absolute right-0 mt-2 w-68 rounded-2xl bg-white dark:bg-slate-900 shadow-2xl border border-slate-200 dark:border-slate-800 p-2 z-50 text-slate-800 dark:text-slate-100"
            >
              ...
              <div className="space-y-1">
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-between p-2.5 rounded-xl text-left text-xs font-medium transition-colors ${
                    isVendor ? 'bg-amber-50 dark:bg-amber-950/50 text-amber-700 dark:text-amber-300 font-semibold' : 'hover:bg-slate-100 dark:hover:bg-slate-800'
                  }`}
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <p className="font-semibold text-slate-900 dark:text-white">Seller / Vendor</p>
                      <p className="text-[10px] text-slate-500">Kofi Tech (Store Portal &amp; Uploads)</p>
                    </div>
                  </div>
                  {isVendor && <Check className="w-4 h-4 text-amber-600" />}
                </button>
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-between p-2.5 rounded-xl text-left text-xs font-medium transition-colors ${
                    user && !isAdmin && !isVendor ? 'bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300 font-semibold' : 'hover:bg-slate-100 dark:hover:bg-slate-800'
                  }`}
                >
                  <div className="flex items-center gap-2.5">
                  ...
                  {user && !isAdmin && !isVendor && <Check className="w-4 h-4 text-emerald-600" />}
                </button>
