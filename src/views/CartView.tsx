  ...
  if (cart.length === 0) {
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32 text-center">
        ...
        <h2 className="text-2xl font-black text-slate-900 dark:text-white mb-3">Your Shopping Bag is Empty</h2>
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-8">
          Looks like you have not added anything yet. Start exploring our catalog!
        </p>
        <button
          onClick={() => onNavigate('shop')}
          className="inline-flex items-center gap-2 px-8 py-3.5 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm shadow-lg shadow-emerald-600/25 transition-all active:scale-95"
        >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-900/40 flex items-center gap-3 text-xs text-emerald-900 dark:text-emerald-200">
          ...
          {subtotal >= settings.freeDeliveryThreshold ? (
            <span>
              Congratulations! Your order qualifies for <strong>FREE Delivery</strong> in Accra and Tema.
            </span>
          ) : (
            <span>
              Add <strong>{formatPrice(settings.freeDeliveryThreshold - subtotal)}</strong> more to get <strong>FREE Delivery</strong>!
            </span>
          )}
        </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-5">
              ...
              <div className="pt-2 flex items-center justify-center gap-2 text-[11px] text-slate-400">
                ...
                <span>100% Encrypted and Secure Checkout</span>
              </div>
            </div>
          </div>
        </div>
